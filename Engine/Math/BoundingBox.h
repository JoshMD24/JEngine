...
struct BoundingBox
{
	...
	inline bool Intersects(BoundingBox* box_)
	{
		...
		//Return result of box box collision

		return (minCorner.x <= otherMaxCorner.x && maxCorner.x >= otherMinCorner.x) && (minCorner.y <= otherMaxCorner.y && maxCorner.y >= otherMinCorner.y) &&
