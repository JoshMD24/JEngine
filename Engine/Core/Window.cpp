...
bool Window::OnCreate(std::string name_, int width_, int height_)
{
	if (SDL_Init(SDL_INIT_VIDEO) < 0)
	{
		...
		Debug::FatelError("Did not initialize SDL", "Window.cpp", __LINE__);
		return false;
	}
	...
	if (!window)
	{
		...
		Debug::FatelError("Failed to create window", "Window.cpp", __LINE__);
		return false;
	}
	...
	if (error != GLEW_OK)
	{
		...
		Debug::FatelError("Failed to initialize GLEW", "Window.cpp", __LINE__);
		return false;
	}
